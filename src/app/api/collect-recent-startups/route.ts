...
import { RecentStartupsCollector } from '@/lib/data-collectors/recent-startups-collector'
import { AIContentGenerator } from '@/lib/ai/content-generator'
import { supabaseAdmin } from '@/lib/supabase/client'

const recentStartupsCollector = new RecentStartupsCollector()
...
export async function POST() {
  try {
    ...
    console.log('🚀 Collecting recently founded startups with funding...')
    ...
    // Collect recent startups
    await recentStartupsCollector.processRecentStartups()
    ...
    return NextResponse.json({
      success: true,
      message: 'Recent startups collection and story generation completed',
      storiesGenerated,
      finalCounts: {
    ...
    console.error('Recent startups collection error:', error)
    return NextResponse.json({
      error: 'Failed to collect recent startups data',
...
export async function GET() {
  try {
    ...
    // Get recent startup statistics
    const { data: recentStartups } = await supabaseAdmin
      .from('startups')
      .select('funding_amount, funding_stage, industry, founded_date')
      .gte('funding_amount', 500000)
      .gte('founded_date', '2019-01-01')
    ...
    return NextResponse.json({
      recentStartups: {
        totalRecentStartups: recentStartups?.length || 0,
        fundingRanges,
        foundingYears,
    ...
    console.error('Recent startups stats error:', error)
    return NextResponse.json({
      error: 'Failed to get recent startups statistics',
      details: error instanceof Error ? error.message : 'Unknown error'
    }, { status: 500 })
  }
} 